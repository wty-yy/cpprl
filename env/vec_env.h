...
struct VecEnvInfo {
  int num_envs;
  torch::Tensor obs, reward, done;
  VecEnvInfo(std::vector<EnvInfo>& infos) : num_envs(infos.size()) {
    std::vector<torch::Tensor> obs_vector;
    std::vector<float> reward_vector;
    std::vector<float> done_vector;
    for (int i = 0; i < num_envs; i++) {
      obs_vector.push_back(infos[i].obs);
...
class VecEnv {
 public:
  ...
  VecEnvInfo reset() {
    ...
    return VecEnvInfo(infos);
  }

  VecEnvInfo step(const std::vector<int>& actions) {
    std::vector<EnvInfo> infos(numEnvs);
  ...
  std::pair<EnvSpace, EnvSpace> get_space() { return envs[0]->get_space(); }
