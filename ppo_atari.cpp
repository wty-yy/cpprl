...
struct Config {
  ...
  std::string path_rom_dir = "/home/yy/Coding/datasets/roms";
  std::string game_name = "breakout";
  int cuda = true;
  int seed = 1;
  int torch_deterministic = true;
  double total_steps = 1e7;
  double init_lr = 2.5e-4;
  double final_lr = 0.0;
  int num_envs = 8;
  int num_steps = 128;
  double anneal_steps = 1e7;
  double gamma = 0.99;
  ...
  Config(int argc, char* argv[]) {
    ...
    parser.add_argument("--version").store_into(version).help("The version of training.");
    parser.add_argument("--path-rom-dir").store_into(path_rom_dir).help("The directory path of the ALE roms.");
    parser.add_argument("--game-name").store_into(game_name).help("The name of the Atari game, file name in [rom-path]");
    ...
    path_rom = fs::path(path_rom_dir) / (game_name + ".bin");
  }
};
...
int main(int argc, char* argv[]) {
  ...
  // Build environment
  VecEnv venv([cfg](int i){return std::make_shared<ALEGame>(
    ALEGameOption(cfg.path_rom).seed(cfg.seed+i)
  );}, cfg.num_envs);
  auto [obs_space, action_space] = venv.get_space();
  // Build model, optimizer
  auto device(torch::cuda::is_available() && cfg.cuda ? torch::kCUDA : torch::kCPU);
  CNN model(obs_space.shape, action_space.n);
  initialize_cnn_weights(model);
  model->to(device);
  ...
  auto obs_shape = obs_space.shape;
  auto obs = torch::zeros({cfg.num_steps, cfg.num_envs, obs_shape[0], obs_shape[1], obs_shape[2]}).to(device);
  ...
  auto next_obs = infos.obs.to(device);
  ...
  for (int iteration = 1; iteration <= cfg.num_iterations + 1; ++iteration) {
    ...
    double mean_reward = 0, mean_legnth = 0, done_count = 0;
    for (int step = 0; step < cfg.num_steps; ++step) {
      ...
      next_obs = infos.obs.to(device);
      next_done = infos.done.to(device);
      rewards[step] = infos.reward;
      for (int i = 0; i < cfg.num_envs; ++i) {
        total_reward[i] += infos.reward[i].item<double>();
        total_length[i] += 1;
        if (infos.done[i].item<bool>()) {
    ...
    auto b_obs = obs.view({-1, obs_shape[0], obs_shape[1], obs_shape[2]});
